...
| 素材管理 | 支持素材录入、素材列表、类型维护、文件夹树、右键创建目录、文件上传和预览。 |
...
如需切换为又拍云存储，可在系统设置的「存储桶」页面选择「又拍云」，并填写服务名、操作员、操作员密码、CDN 域名和上传目录。生产环境不要把真实密钥提交到代码仓库。
...
- 如启用又拍云，确认服务名、操作员权限、CDN 域名和上传目录已配置。

### 手动覆盖部署提醒

如果服务器使用源码部署，后端只覆盖本次更新涉及的代码文件并重启服务；不要覆盖线上 `.env`、SQLite 数据库、上传目录和日志目录。

如果服务器使用前端静态资源部署，建议在本地执行：
